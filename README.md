...
These screenshots were captured from the local Next.js application running against the FastAPI API after processing the generated synthetic ramp video. The records are demo/sample data.
...
2. Generate `sample-data/synthetic-ramp.mp4`.
3. Upload the MP4 through the API or SDK using the `synthetic_color` detector.
...
job = client.upload_video(
    ...
    path="sample-data/synthetic-ramp.mp4",
    detector_backend="synthetic_color",
)
...
py -3.12 scripts/benchmark.py sample-data/synthetic-ramp.mp4
...
AeroRamp Vision builds on open-source libraries including FastAPI, SQLAlchemy, Alembic, Pydantic, OpenCV, Shapely, PyJWT, Uvicorn, Prometheus Client, Next.js, React, and TypeScript. Optional YOLO workflows use Ultralytics when installed by the user. The bundled demo video is generated locally by `scripts/generate_synthetic_video.py`.
